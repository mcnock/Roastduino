...
//we read current each loop since it is fast
 int tempread;
  ...
  if (bNewSecond) { //we speed up loop per sec by reading temps once per second.  they are very slow.
...
//  }
  ...
  if (State == AMROASTING) {
    ...
    if (RoastMinutes > SpanMinutesLength[2] ) { //only calc intergral error if we are above the 1st setup
