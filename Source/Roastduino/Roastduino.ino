...
void loop () {
  ...
  if (State == AMROASTING) {
    ...
    //this mean a new window or we are less the the window size time duty we are on.
    if ((PIDNewWindow == true && Duty > 0) || ( (now - PIDWindowStartTime) <=  (Duty * PIDWindowSize))) {
      //Serial.println ("ssd 1 is always off.  sdd2 with be proporinAL");
      digitalWrite(PIDSSD1p, HIGH);
      //if (SSD1OnTime == 0) {
      //  Serial.println ("start ssd1on  timer. this records how long we have been in zone high");
      //  SSD1OnTime = now;
      //}
      //else if ((now - SSD1OnTime) > 0000) {
    ...
    } else
    {
      ...
      if ((PIDNewWindow == true & Duty <= 0) || ( (now - PIDWindowStartTime)) <=  ((1 + Duty) * PIDWindowSize))) {
        {
        //  if (SSD2OffTime == 0) {
          //  Serial.println ("ssd 1 off 5 sec timer started.  This how long we are zone low");
         //   SSD2OffTime = now;
         // }
         // else if ((now - SSD2OffTime) > 0000) {
            digitalWrite(PIDSSD1p, HIGH);
          //  SSD1OnTime = 0;
         // }
        }
